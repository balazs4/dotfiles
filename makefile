...
$(HOME)/%: %
	...
	@cat $(<) \
		...
		| sed "$$(cat .colors)" \
		| tee $(@) > /dev/null
	@$(if $(filter $<, .config/spotify-player/app.toml), sed 's|__CLIENT_ID__|$(SPOTIFY_CLIENT_ID)|' -i $(@)                                                                              || true)
...
.PHONY: sync
sync:
	printf "%s@%s" $(shell date +%s) $(shell cat /etc/hostname) | xargs -t -I{} git commit -am "{}" || true
	git pull || true
	git push || true
...
.colors: $(HOME)/.cache/schemes
	@git -C $(HOME)/.cache/schemes ls-files \
		| sort \
		| fzf --reverse --height=45% --ansi --preview-window=nohidden -q "'yaml 'base16 " --preview 'cat $(HOME)/.cache/schemes/{} | yq ".palette[]" | xargs bash .local/bin/hex2ansi' \
